...
export const Experience = () => {
  ...
  const CURRENT_AHEAD_CAMERA = 0.0008
  const CURRENT_AHEAD_AIRPLANE = 0.02
  const AIRPLANE_MAX_ANGLE = 35
  const FRICTION_DISTANCE = 45
  ...
  useFrame((_state, delta) => {
    ...
    textSections.forEach((textSection) => {
      ...
      if (distance < FRICTION_DISTANCE) {
          friction = Math.max(distance / FRICTION_DISTANCE, 0.1); // Calculate friction for each text section
            ...
            cameraRail.current.position.lerp(targetCameraRailPosition, delta ); // Apply friction to camera movement
    ...
    cameraGroup.current.position.lerp(curPoint, delta * 24 ); // Apply friction to camera movement

    const lookAtPoint = curve.getPoint(Math.min(lerpedScrollOffset + CURRENT_AHEAD_CAMERA, 1));

    const currentLookAt = cameraGroup.current.getWorldDirection(
        new THREE.Vector3()
    );
    const targetLookAt = new THREE.Vector3()
        .subVectors(curPoint, lookAtPoint).normalize();
    ...
    const tangent = curve.getTangent(lerpedScrollOffset + CURRENT_AHEAD_AIRPLANE);
  ...
  return (
    ...
    <directionalLight  position={[0,3,1]} intensity={0.1}/>
      ...
      <mesh>
        ...
        <meshStandardMaterial ref={lineMaterialRef} onBeforeCompile={fadeOnBeforeCompile} color={"white"} opacity={0.7} envMapIntensity={2} transparent/>
      </mesh>
      </group>
        {
          clouds.map((cloud,index)=>(
            <Cloud sceneOpacity={sceneOpacity} scale={[1.5, 1.5,1.5]} {...cloud} key={index} />
          ))
        }
    </>
  );
